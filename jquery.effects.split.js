...
var defaultOptions ={
    ...
    random: false, //float
    interval: false, //Number, miliseconds before each piece animation (optional)
    fade: true, //Boolean, indicates if the pieces should fade.
    rows: 5, //Number of rows
    cols: 5, //Number of rows
    crop: false //Boolean, show pieces outside of the main element.
};
...
$.effects.splitExplode = function(o,show){
    ...
    var options = o.options = $.extend({}, 
					   ...
					   animate: function(interval,duration,x,y,parentCoords){
						   ...
						   //delay = ((show && options.sync) || (!show && !options.sync)) ? (options.duration - duration) : 0;
						   
						   randomX = Math.random() - 0.5;
						   randomY = Math.random() - 0.5;
					       }
						   
					       var distanceY = ((parentCoords.height - height) / 2 - height * y),
						   distanceX = ((parentCoords.width - width) / 2 - width * x),
						   distanceXY = Math.sqrt(Math.pow(distanceX, 2) + Math.pow(distanceY, 2)),
					           offsetTo = {
                                                       top: parseInt(offset.top - distanceY * distance + distanceXY * randomY),
						       left: parseInt(offset.left - distanceX * distance + distanceXY * randomX)
                                                   };
    ...
    var options = o.options = $.extend({}, 
					   ...
					   animate: function(interval,duration,x,y,parentCoords){
				                   ...
				                   rowOdd = colOdd;
                                               ...
                                               if (colOdd) {
				                   if (rowOdd) {
					               properties.top = properties.top + height * options.distance;
				                   } else {
					               properties.left = properties.left + width * options.distance;
				                   }
			                       }

			                       if (colOdd != rowOdd) {
				                   properties.width =  width * (1 - options.distance);
			                       } else {
				                   properties.height = height * (1 - options.distance);
			                       };
    ...
    var options = o.options = $.extend({}, 
					   ...
					   starting: !show ? starting : null,
						   ...
						   left = left < 0 ? 0 : left;
                                               ...
                                               if (options.direction.indexOf('right') || options.direction.indexOf('left'))
                                                  show ? this.css('left',left) : properties.left = left;
                                               if (options.direction.indexOf('top') || options.direction.indexOf('bottom'))
                                                  show ? this.css('top',top) : properties.top = top;
    ...
    var options = o.options = $.extend({}, 
					   ...
					   starting: !show ? starting : null,
				                   ...
				                   rowOdd = !colOdd;
                                               ...
                                               if (colOdd == rowOdd) {
                                                   properties.left = !colOdd ? offset.left - distanceX : offset.left + distanceX;
                                               } else {
                                                   properties.top = !colOdd ? offset.top - distanceY : offset.top + distanceY; 
                                               }

                                               if (show){
                                                   //Bug: it should work just by switching properties and offset but somehow this won't work. therefore we takes and configure a new offset.
    ...
    return this.queue(
        function(){
            var $this = $(this),
                ...
                parentCoords = $.extend(position, {width: width, height: height}),
            ...
            for(y = 0; y < ly; y++){
                for(x = 0; x < lx; x++){
                    ...
                    $.type(options.beforeAnimate) === 'function' && options.beforeAnimate.call($piece);
                    
                    // call the animation for each piece.
                    options.animate.call($piece, interval, duration, x, y, parentCoords);
                }
            }
